...
@testset "3D MeshData tests for wedges and pyramids" begin
    ...
    @testset "Initialization test for N = 1 for $element_type" for element_type in [
        Pyr(),
        Wedge()
    ]
        K1D = 2
    ...
    @testset "$approximation_type $element_type MeshData initialization" for (approximation_type, element_type) in approx_elem_types_to_test
        ...
        u = @. sin(pi * (0.5 + x)) * sin(pi * (0.5 + y)) * sin(pi * (0.5 + z))
        (; mapP) = md_periodic
        uf = Vf * u
        @test uf ≈ uf[mapP]
    ...
    @testset "TensorProductWedge MeshData" begin
        element_type = Wedge()
        tol = 5e2 * eps()
        @testset "Degree $tri_grad triangle" for tri_grad in [2, 3]
            @testset "Type $tri_type triangle" for tri_type in [SBP(), Polynomial()]
                @testset "Degree $line_grad line" for line_grad in [2, 3]
                    @testset "Type $line_type line" for line_type in [SBP(), Polynomial()]
                        line = RefElemData(Line(), line_type, line_grad)
                        tri = RefElemData(Tri(), tri_type, tri_grad)
                        ...
                        @test StartUpDG._short_typeof(rd.approximation_type) ==
                              "TensorProductWedge{$(StartUpDG._short_typeof(line.approximation_type)), $(StartUpDG._short_typeof(tri.approximation_type))}"
                        @test typeof(md.mesh_type) <:
                              StartUpDG.VertexMappedMesh{<:typeof(rd.element_type)}
                        @test md.x == md.xyz[1]
                        @test md.y == md.xyz[2]
                        @test md.z == md.xyz[3]
