...
function compute_neighbor_list(md; threshold_score=VolumeScore(-Inf), score_params=md)

    if threshold_score.value == -Inf
        default_threshold_score!(threshold_score, score_params)
    end
    ...
    for e in 1:num_cut_elements(md)
        ...
        while length(neigbors) > 0 && !is_better(merged_score, threshold_score)
            ...
            compute_nbhd_score!(best_score, vcat(merge_nbhds[e], neighbors[1]), score_params)
            
            for e_nbhr in neighbors[2:end]
                compute_nbhd_score!(new_score, vcat(merge_nbhds[e], e_nbhr), score_params)
