...
# constructs MeshData for a hybrid mesh given a LittleDict of `RefElemData` 
# with element type keys (e.g., `Tri()` or `Quad`). 
function MeshData(VX, VY, VZ, EToV_unsorted, rds::LittleDict{AbstractElemShape, <:RefElemData};
    is_periodic = (false, false))
...
for elem_type in element_types
    eids = element_ids[elem_type]
    x, y, z = xyz_hybrid[elem_type]
    @unpack V1 = rds[elem_type]
    for (e_local, e) in enumerate(eids)
        etov = EToV[e]        
        x[:, e_local] .= vec(V1 * VX[etov'])
        y[:, e_local] .= vec(V1 * VY[etov'])
        z[:, e_local] .= vec(V1 * VZ[etov'])
    end
end    
...
return MeshData(HybridMesh(Tuple(element_types)), (VX, VY, VZ), EToV, FToF, 
      xyz, xyzf, xyzq, wJq,
      mapM, mapP, mapB,
      rstxyzJ, J, nxyzJ, Jf, 
      is_periodic)
