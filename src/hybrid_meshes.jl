...
struct HybridMesh{T, TV, TE}
    ...
    function HybridMesh(element_types::T, VXYZ, EToV) where {T <: Tuple}
        ...
        return new{typeof(Tuple(element_types[p])), typeof(VXYZ), typeof(EToV)}(Tuple(element_types[p]),
                                                                                VXYZ, EToV)
    end
end
...
@inline getproperty(rds::MultipleRefElemData, s::Symbol) = getproperty(getfield(rds, :data),
                                                                       s)
...
function connect_mesh(EToV::AbstractVector{<:AbstractArray},
    ...
    elem_types = Tuple(getproperty(rds, key).element_type
                       for key in keys(face_vertex_indices))
...
function RefElemData(element_types::NTuple{NT, Union{Tri, Quad}}, N::Int;
    ...
    rds = NamedTuple((typename(elem) => RefElemData(elem, N; quad_rule_face, kwargs...)
                      for elem in element_types))

    # check if number of face nodes is the same 
    num_face_nodes = length.(getproperty.(values(rds), :rf)) .÷
                     num_faces.(getproperty.(values(rds), :element_type))
...
function MeshData(mesh::Tuple{<:Tuple, Vector{Matrix{Int64}}}, rd::MultipleRefElemData,
                  other_args...)
    MeshData(mesh..., rd; other_args...)
end
function MeshData(VXYZ::Tuple, EToV, rd::MultipleRefElemData, other_args...)
    MeshData(VXYZ..., EToV, rd; other_args...)
end # splats VXYZ 
...
function MeshData(VX, VY, EToV_unsorted,
    ...
    element_ids = NamedTuple((Pair(typename(elem),
                                   findall(length.(EToV) .== num_vertices(elem)))
                              for elem in element_types))
    num_elements_of_type(elem) = length(getproperty(element_ids, typename(elem)))

    # make node arrays 
    allocate_node_arrays(num_rows, element_type) = ntuple(_ -> zeros(num_rows,
                                                                     num_elements_of_type(element_type)),
                                                          ndims(element_type))

    xyz_hybrid = NamedTuple((typename(rd.element_type) => allocate_node_arrays(size(rd.V1,
                                                                                    1),
                                                                               rd.element_type)
                             for rd in values(rds)))
    ...
    n_dims = 2
    xyz = ntuple(i -> NamedArrayPartition(NamedTuple(Pair.(keys(rds),
                                                           getindex.(values(xyz_hybrid), i)))),
                 n_dims)
    xyzf = ntuple(i -> NamedArrayPartition(NamedTuple(Pair.(keys(rds),
                                                            getindex.(getproperty.(geo,
                                                                                   :xyzf),
                                                                      i)))), n_dims)
    xyzq = ntuple(i -> NamedArrayPartition(NamedTuple(Pair.(keys(rds),
                                                            getindex.(getproperty.(geo,
                                                                                   :xyzq),
                                                                      i)))), n_dims)

    # 4 entries in the geometric term matrix for 2D hybrid meshes
    rstxyzJ = ntuple(i -> NamedArrayPartition(NamedTuple(Pair.(keys(rds),
                                                               getindex.(getproperty.(geo,
                                                                                      :rstxyzJ),
                                                                         i)))),
                     n_dims * n_dims)
    ...
    nxyzJ = ntuple(i -> NamedArrayPartition(NamedTuple(Pair.(keys(rds),
                                                             getindex.(getproperty.(geo,
                                                                                    :nxyzJ),
                                                                       i)))), n_dims)
...
function MeshData(rds::MultipleRefElemData,
    ...
    xyz = Tuple(NamedArrayPartition(NamedTuple(zip(keys(rds),
                                                   (getproperty(tuple_fields[rd.element_type],
                                                                :xyz)[dim]
                                                    for rd in values(rds)))))
                for dim in 1:Dim)
    xyzf = Tuple(NamedArrayPartition(NamedTuple(zip(keys(rds),
                                                    (getproperty(tuple_fields[rd.element_type],
                                                                 :xyzf)[dim]
                                                     for rd in values(rds)))))
                 for dim in 1:Dim)
    xyzq = Tuple(NamedArrayPartition(NamedTuple(zip(keys(rds),
                                                    (getproperty(tuple_fields[rd.element_type],
                                                                 :xyzq)[dim]
                                                     for rd in values(rds)))))
                 for dim in 1:Dim)
    nxyzJ = Tuple(NamedArrayPartition(NamedTuple(zip(keys(rds),
                                                     (getproperty(tuple_fields[rd.element_type],
                                                                  :nxyzJ)[dim]
                                                      for rd in values(rds)))))
                  for dim in 1:Dim)
    rstxyzJ = SMatrix{Dim, Dim}(NamedArrayPartition(NamedTuple(zip(keys(rds),
                                                                   (getproperty(tuple_fields[rd.element_type],
                                                                                :rstxyzJ)[dim]
                                                                    for rd in values(rds)))))
                                for dim in 1:(Dim * Dim))

    J = NamedArrayPartition(NamedTuple(zip(keys(rds),
                                           (getproperty(scalar_fields[rd.element_type], :J)
                                            for rd in values(rds)))))
    wJq = NamedArrayPartition(NamedTuple(zip(keys(rds),
                                             (getproperty(scalar_fields[rd.element_type],
                                                          :wJq) for rd in values(rds)))))
    Jf = NamedArrayPartition(NamedTuple(zip(keys(rds),
                                            (getproperty(scalar_fields[rd.element_type],
                                                         :Jf) for rd in values(rds)))))
