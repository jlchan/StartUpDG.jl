...
function RefElemData(elem::Union{Line, Tri, Tet, Wedge, Pyr},
...
function RefElemData(elem::Union{Quad, Hex},
                     approximation_type::Polynomial{DefaultPolynomialType}, N; kwargs...)
    RefElemData(elem, Polynomial(TensorProductQuadrature(gauss_quad(0, 0, N))), N;
                kwargs...)
end

# special case: for lines, tensor product and multidimensional quadrature are the same
function RefElemData(elem::Line, approx_type::Polynomial{<:TensorProductQuadrature}, N;
                     kwargs...)
...
function RefElemData(elem::Hex,
    ...
    # quadrature nodes - build from 1D nodes.    
    rq, sq, tq, wq = tensor_product_quadrature(elem,
                                               approximation_type.data.quad_rule_1D...)
...
function RefElemData(elem::Hex, approximation_parameter::TensorProductQuadrature, N;
                     Nplot = 10)
...
```
"""
function RefElemData(elem::Union{Tri, Tet, Pyr},
                     approx_type::Polynomial{<:TensorProductQuadrature}, N; kwargs...)
