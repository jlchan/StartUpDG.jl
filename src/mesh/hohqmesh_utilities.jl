...
function MeshData(hmd::HOHQMeshData{2}, rd::RefElemData)
    ...
    chebyshev_to_lobatto = vandermonde(Line(), hmd.polydeg, nodes(Line(), rd.N)) /
                           vandermonde(Line(), hmd.polydeg, r_chebyshev)

    warp_face_nodes_to_volume_nodes = face_basis(rd.element_type, rd.N, rd.rst...) /
                                      face_basis(rd.element_type, rd.N, rd.r[rd.Fmask],
                                                 rd.s[rd.Fmask])

    HOHQMesh_to_StartUpDG_face_ordering = get_HOHQMesh_to_StartUpDG_face_ordering(rd.element_type)
    curved_face_coordinates = ntuple(_ -> similar(reshape(md.x[rd.Fmask, 1], :,
                                                          rd.num_faces)), 2)
    x, y = copy.(md.xyz)
    for curved_elem in hmd.curved_elements
        ...
        for (f_HOHQMesh, f) in enumerate(HOHQMesh_to_StartUpDG_face_ordering)
            if curved_faces[f_HOHQMesh] == 1
                ids_HOHQMesh = get_HOHQMesh_ids(rd.element_type, curved_faces, f_HOHQMesh,
                                                f, hmd.polydeg)

                curved_lobatto_coordinates = chebyshev_to_lobatto *
                                             curved_edge_coordinates[ids_HOHQMesh, :]
    ...
    face_indices = [map(x -> global_face_index(x, rd.num_faces,
                                               HOHQMesh_to_StartUpDG_face_ordering),
...
function global_face_index(local_elem_and_face::CartesianIndex, num_faces_per_element,
                           face_ordering)
...
function MeshData(hmd::HOHQMeshData{3}, rd::RefElemData{3, <:Hex})
    ...
    # interpolation matrix from chebyshev_to_lobatto nodes
    r_chebyshev = vec([-cos(j * pi / hmd.polydeg)
                       for j in 0:(hmd.polydeg), i in 0:(hmd.polydeg)])
    s_chebyshev = vec([-cos(j * pi / hmd.polydeg)
                       for i in 0:(hmd.polydeg), j in 0:(hmd.polydeg)])

    chebyshev_to_lobatto = vandermonde(face_type(rd.element_type), hmd.polydeg,
                                       nodes(face_type(rd.element_type), rd.N)...) /
                           vandermonde(face_type(rd.element_type), hmd.polydeg, r_chebyshev,
                                       s_chebyshev)

    warp_face_nodes_to_volume_nodes = face_basis(rd.element_type, rd.N, rd.rst...) /
                                      face_basis(rd.element_type, rd.N, rd.r[rd.Fmask],
                                                 rd.s[rd.Fmask], rd.t[rd.Fmask])
...
function MeshData(hmd::HOHQMeshData{3}, rd::RefElemData{3, <:Tet})
    ...
    # interpolation matrix from chebyshev_to_lobatto nodes
    a_chebyshev = vec([-cos(j * pi / hmd.polydeg)
                       for j in 0:(hmd.polydeg), i in 0:(hmd.polydeg)])
    b_chebyshev = vec([-cos(j * pi / hmd.polydeg)
                       for i in 0:(hmd.polydeg), j in 0:(hmd.polydeg)])
