...
function t8_forest_leaf_face_neighbors_(forest, itree, eclass_scheme, element, iface)
...
function compute_connectivity(forest)
  ...
  for itree = 0:num_local_trees-1
    ...
    for ielement = 0:num_elements_in_tree-1
      ...
      # local_vertices = Matrix{Cdouble}(undef, 3, num_corners) 
      local_vertices = []
      for corner_number in 1:num_corners 
        corner_vertex = Vector{Cdouble}(undef, 3) 
        t8_forest_element_coordinate(forest, itree, element, corner_number-1, corner_vertex)
        push!(local_vertices, corner_vertex)
      end
      ...
      for iface = 1:num_faces
        face_num_neighbors, face_neighbor_ids, face_neighbors, face_dual_faces, face_orientation =
          t8_forest_leaf_face_neighbors_(forest, itree, eclass_scheme, element, iface-1)
  ...
  return vertices, neighbor_ids, dual_faces, orientations
end
...
vertices, neighbor_ids, dual_faces, orientations = compute_connectivity(forest)
...
function calc_centroid(etype, vertices)
  if etype == :tri
    # Pseudo-centroid.
    # return sum(vertices) ./ 3.0

    # Proper centroid.
    centroid = [0.0, 0.0]
    for (icorner,face_corners) in enumerate(t8_iface_to_corners[etype])
      v0 = vertices[icorner][1:2]
      vA = vertices[face_corners[1]][1:2]
      vB = vertices[face_corners[2]][1:2]
      mp = 0.5 .* (vA + vB) # midpoint of the face
      centroid .+= 0.5 .* (v0 + mp)
    end
    return centroid ./ 3.0
  end
  if etype == :quad
    # Pseudo-centroid. TODO: Compute real centroid.
    return sum(vertices) ./ 4.0
  end

  return Nothing
end

function calc_volume(etype, vertices)
...
function sprint_face_info(iface, ineighs, duals, orient)
  neighs_and_duals = join(["($n,$d)" for (n,d) in zip(ineighs, duals)],",")
  return "$iface\n[$neighs_and_duals]"
end

# ========= Plotting the mesh with annotations =========== #
...
# Loop over elements.
for (ielement, local_vertices) in enumerate(vertices)
  num_corners = length(local_vertices)
  etype = t8_num_corners_to_etype[num_corners]
  volume = calc_volume(etype, local_vertices)
  centroid = calc_centroid(etype, local_vertices)[1:2]

  # Annotate global element id.
  color = volume < 0.0 ? :blue : :red
  annotate!(centroid[1], centroid[2], ("$ielement", "Helvetica Bold",24), color)

  # Loop over element edges/faces.
  for (iface,face_corners) in enumerate(t8_iface_to_corners[etype])
    vA = local_vertices[face_corners[1]][1:2] # vertex A
    vB = local_vertices[face_corners[2]][1:2] # vertex B
    mp = 0.5 .* (vA + vB) # edge/face midpoint
    ...
    plot!(
      ...
      ratio=1.0,
      size=(800,800),
      leg=false, linewidth=2, color=:black
    )

    # Plot face orientation with an arrow.
    weight = 0.8
    vA_ = (1.0 - weight) .* centroid + weight .* vA
    vB_ = (1.0 - weight) .* centroid + weight .* vB
    weight = 0.8
    vA__ = (1.0 - weight) .* vA_ + weight .* 0.5.*(vA_ + vB_)
    vB__ = (1.0 - weight) .* vB_ + weight .* 0.5.*(vA_ + vB_)

    if length(orientations[ielement]) > 0
      color = orientations[ielement][iface] == 0 ? :green : :red
    else
      color = :black
    end
    plot!(
      [vA__[1],vB__[1]], [vA__[2],vB__[2]],
      ratio=1.0,
      arrow=(:closed, 2.0),
      leg=false, linewidth=2, color=color
    )

    nids = neighbor_ids[ielement][iface]
    ndfs = dual_faces[ielement][iface]
    orient = orientations[ielement][iface]

    # Annotate face index.
    weight = 0.5
    midpoint = (1.0 - weight) .* centroid + weight .* mp 
    annotate!(midpoint[1], midpoint[2], sprint_face_info(iface, nids, ndfs, orient), :black)
  end

  # Loop over element corners.
  for (icorner,vertex) in enumerate(local_vertices)
     # Annotate corner id.
    weight = 0.8
    v = (1.0 - weight) .* centroid + weight .* vertex[1:2]
    annotate!(v[1], v[2], "$icorner", :black)
  end
